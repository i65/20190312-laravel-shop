                ...
                <div class="card-body">
                    ...
                    @if($address->id)
                        <form class="form-horizontal" role="form" action="{{ route('user_addresses.update', ['user_address' => $address->id]) }}" method="POST">
                            {{ method_field('PUT') }}
                    @else
                        <form class="form-horizontal" rol="form" action="{{ route('user_addresses.store') }} " method="post">
                    @endif
                        <!-- 引入 csrf token 字段 -->
                            {{ csrf_field() }}
                            <!-- inline-template 代表通过内联方式引入组件 -->
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-9">
                                    <input type="text" class="form-control" name="contact_phone" value="{{ old('contact_name', $address->contact_phone) }}">
                                </div>
                            </div>
