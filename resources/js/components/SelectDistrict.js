...
// 引入 lodash，lodash 是一个实用工具库，提供了很多的常用方法
import _ from 'lodash';

// 注册一个名为 selece-district 的 Vue 组件
Vue.component('select-district', {
    ...
    methods: {
        ...
        setFromValue(value) {
            ...
            // 如果数组长度为 0，则将省清空（由于我们定义了观察器，会联运触发将城市和地区清空）
            if (value.length === 0) {
                this.provinceId = '';
                return;
            }
            ...
            const cityId = _.findKey(addressData[proinceId], o => o === value[1]);
            // 没找到，清空城市的值
            if (!cityId) {
                this.cityId = '';
                return;
